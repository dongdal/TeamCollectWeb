...
<script>
    $('#lnkReport').on('click', function () {
        @*var AgenceId = @ViewBag.UserAgenceId.ToString;*@ 
        var Mois = $('#Mois').val();
        var Annee = $('#Annee').val();
        var AgenceId = $('#AgenceId').val();
        if (Annee == null || Annee == "" || Mois == null || Mois == "" || Annee == null || Annee == "") {
            $.alert("Merci de bien vouloir sélectionner un mois, une année ainsi qu'une agence.");
        } else {
